...
'''
measurement_duration = 30  # Duration to collect data in seconds
...
if main.connected:
    ...
    while time.time() - start_time < measurement_duration:
        ...
        for i, rb in enumerate(range_bins):
            channels_fd_1[rb].append(fd_data[0][i])
            channels_fd_2[rb].append(fd_data[1][i])
            channels_fd_3[rb].append(fd_data[2][i])
            channels_fd_4[rb].append(fd_data[3][i])
...
def plot_fd_range_bins(min_bin, max_bin):
    ...
    for i, rb in enumerate(selected_bins[:8]):
        if channels_fd_1[rb]:
            axs[i].plot(timestamps, channels_fd_1[rb], label="Channel 1")
        if channels_fd_2[rb]:
            axs[i].plot(timestamps, channels_fd_2[rb], label="Channel 2")
        if channels_fd_3[rb]:
            axs[i].plot(timestamps, channels_fd_3[rb], label="Channel 3")
        if channels_fd_4[rb]:
            axs[i].plot(timestamps, channels_fd_4[rb], label="Channel 4")
