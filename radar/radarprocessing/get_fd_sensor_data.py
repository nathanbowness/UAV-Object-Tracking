...
from radar.RadarDevKit.RadarModule import RadarModule, GetRadarModule
...
from radar.radarprocessing import TDData
...
def get_td_data_voltage_T(radar_module: RadarModule, ramp_type: str = "UP-Ramp"):
    """
    Get the TD in units of Voltage.
    (1024, 4) -- I1, Q1, I2, Q2
    """
    return get_td_data_voltage(radar_module, ramp_type).T
...
def get_fd_data_with_angles_from_radar(radar_config : RadarConfiguration, 
                                       radar_module: RadarModule,
                                       ramp_type: str = "UP-Ramp"):
    # Collect data
    radar_module.GetFdData(ramp_type)
    fd_data = get_FD_data_angle(radarModule=radar_module, radarParams=radar_module.sysParams)
...
def get_fd_data_from_radar(radar_config : RadarConfiguration, 
                           radar_module: RadarModule,
                           ramp_type: str = "UP-Ramp") -> FDDataMatrix:
    # Collect data
    radar_module.GetFdData(ramp_type)
    fd_data = get_FD_data_phase_data(radar_module, radarParams=radar_module.sysParams)
    
    # Print the FD data
    if radar_config.run_type == RunType.LIVE_RECORD:
        fd_data.print_data_to_file(radar_config.output_path)
    
    return fd_data

if __name__ == "__main__":  
    radar_config = RadarConfiguration()
    module = radar_config.connect_get_radar_module
    
    test = get_fd_data_from_radar(radar_config, module)
    print("test")
